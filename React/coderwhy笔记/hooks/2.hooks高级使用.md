...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)useEffect和useLayoutEffect对比
...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)image-20200729101732525
...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)image-20200729145143429

点到useState的源码中：

- useState本质上，是使用的dispatcher的useState；

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4LNChryibLVjW4wA9dvFfzsmjqAEKJkoricy7ovykVicw16DoVRGFoLIQg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729145217786

dispatcher来自另外的一个函数`resolveDispatcher`：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4pJCggBOYvo1TuW49sNSCLl50usnvKibsXezFA1ntaqrMmRtmEPH7IDg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729145350575

运行的过程中，会赋值一个current的值是一个Dispatcher类型：

- Dispatcher来自于 `react-reconciler/src/ReactFiberHooks`

![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)image-20200729145507410

Dispatch类型的定义：

![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)Dispatch类型的定义
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4DsE73iciam553StVW68jSfiaEKtYp5x9c9KQB69ggofn2u015N7PKyMZg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729150204035

挂载哪一个取决于renderWithHook函数：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4gjZrDzjgFm7BIEfJKtIzJBrMlIIWRjC0Atd8tFTiaJmLFibqBNgZiaF9A/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729151949009

### 3.2. HooksDispatcherOnMount

HooksDispatcherOnMount对应的useState是mountState

![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)image-20200729152902434

mountState的源码：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC49iaKUzvpSMxdaLJL8IicIqHQLZNBobxwA1eRLQ6j41auQwuPfXbPDpDw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)mountState的源码

绑定的dispatchAction函数，事实上是将所有的action放到了queue的队列中：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4uE3Ppfl19kPPPAVEOIDOsg2wCGqPSEWkXDUIF9upyvqsIFkKSsop9Q/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)dispatchAction函数

### 3.3. HooksDispatcherOnUpdate

HooksDispatcherOnUpdate对应的useState是updateState

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4NDqyGJ8ZXMEsibNGGARtOZl61z0S8ia9iadKFPFGYkREAyVWh6BAxpAhQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729161113111

updateState本质上会执行updateReducer：

- 所有其实就更新阶段而言，useState本质上用的是updateReducer

![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)image-20200729161236197

updateReducer的源码如下：

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4k120bU7q7Oy4pVYsdIibqWuwCXRoSiclppnVLDK6RaumXRI0q9TKicRRw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729161529802

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/O8xWXzAqXuvA7ErxSngSDSOVxnyw1rC4pTV9N5lIaRdf1vI3ScMGLxkz8DwYTmvY9AdpMicUJgViaSXwC7PHksGQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)image-20200729161941808
